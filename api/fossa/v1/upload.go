...
type V1UploadBody struct {
	...
	Analysis []fossa.SourceUnit
}

// Upload uploads a project's analysis.
func Upload(uploadBody V1UploadBody) (fossa.Locator, error) {
	...
	res, statusCode, err := fossa.Post("/api/cli/v1/build", payload)
	...
	if err != nil {
		return fossa.Locator{}, errors.Errorf("Could not unmarshal response from API: %s", res)
	}
	if unmarshalled.Error != "" {
		return fossa.Locator{}, errors.Errorf("API return an error on upload: %s", unmarshalled.Error)
	}
