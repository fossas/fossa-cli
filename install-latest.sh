...
http_download_curl() {
  ...
  echo "running: curl -w '%{HTTP_CODE}' -sL $header -H "Cache-Control: no-cache" -o "$local_file" "$source_url""
...
http_download_wget() {
  ...
  echo "running: wget -q $header --no-cache --server-response -O "$local_file" "$source_url" 2>&1 | awk 'NR==1{print $2}'"
