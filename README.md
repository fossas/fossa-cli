...
To output the dependency report to a file run `fossa -o <filename>`. You can also pass a custom output template to use `fossa -t deps.tpl -o deps.txt`
...
fossa report --type licenses -o NOTICE.txt
```

#### Report Templates

You can create a custom report template and pass it to the report generator.
The default licence template is
```
