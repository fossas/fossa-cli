...
analyze = Diag.context "fossa-analyze" $ do
  ...
  vsiResults <-
    Diag.context "analyze-vsi" $
      if (fromFlag Config.VSIAnalysis $ Config.vsiAnalysisEnabled $ Config.vsiOptions cfg)
        then analyzeVSI apiOpts basedir filters skipResolutionSet
        else pure Nothing
  binarySearchResults <-
...
analyzeVSI ::
  ...
  VSI.SkipResolution ->
  m (Maybe SourceUnit)
analyzeVSI (Just apiOpts) dir filters skipResolving = do
