...
buildGraph lockFile = withoutLocalPackages $
  run . evalGrapher $ do
    ...
    --
    -- In this case:
    --   1. aws-sdk@2.1148.0 is deep
    --   2. aws-sdk@2.1148.0 -> buffer@4.9.2
    --   3. aws-sdk@2.1148.0 -> events@1.1.1
    --
    for_ (toList $ packages lockFile) $ \(pkgKey, pkgMeta) -> do
      -- Skip if package is local
  ...
  where
    ...
    getPkgNameVersion = case lockFileVersion lockFile of
      ...
      PnpmLockLt4 _ -> getPkgNameVersionV5
    ...
    getPackageVersion name version =
      if "workspace:" `Text.isPrefixOf` version
        then do
