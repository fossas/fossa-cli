...
import Strategy.AlpineLinux.Types (AlpinePackage (alpinePackageArchitecture, alpinePackageName, alpinePackageVersion))
...
buildGraph pkgs = directs (map toDependency pkgs)
  where
    ...
    toDependency pkg =
      Dependency LinuxAPK (alpinePackageName pkg) (Just $ version pkg) mempty mempty mempty
