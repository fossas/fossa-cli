...
discover' = walk $ \_ _ files ->
  case find (\f -> fileName f == "Gemfile.lock") files of
...
strategy= Strategy
  { strategyName = "ruby-gemfilelock"
  , strategyAnalyze = \opts -> analyze & fileInputJson (targetFile opts)
...
analyze :: Member (Input GemfileLock) r => Sem r G.Graph
