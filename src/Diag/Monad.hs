...
ResultT ma <||> ResultT ma' = ResultT $ do
  ...
  case resA of
    ...
    Failure ws eg -> do
      resA' <- ma'
      case resA' of
        Success ws' a' -> pure (Success (ws' <> failureToWarnings ws eg) a')
...
recoverR (Failure ws eg) = Success (failureToWarnings ws eg) Nothing
...
-- Turn (emitted warnings * errgroup) into (emitted warnings)
--
-- The current semantics are that the ErrGroup gets prepended to the list of
-- warnings iff warnings are attached to it; otherwise it's dropped
failureToWarnings :: [EmittedWarn] -> ErrGroup -> [EmittedWarn]
failureToWarnings ws (ErrGroup sws ectx es) =
  -- FIXME: errors from errgroup get dropped when no warning is present. Are these the semantics we want?
  case NE.nonEmpty sws of
    Nothing -> ws
    Just sws' -> (WarnOnErrGroup sws' ectx es : ws)
{-# INLINE failureToWarnings #-}
