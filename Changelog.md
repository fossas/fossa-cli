...
- Maven: static analysis (`pomxml`, used whenever `mvn` is unavailable, e.g. by hosted/GitHub App imports) no longer reports the project's own artifact as the only Direct dependency with every declared dependency demoted to Transitive; the static path now removes the project artifact and promotes declared dependencies to Direct, matching dynamic analysis.
