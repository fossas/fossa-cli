...
- Pnpm: multi-document `pnpm-lock.yaml` files (written by pnpm v11, which prepends a metadata document to the lockfile) no longer fail analysis with `Multiple YAML documents encountered`.
