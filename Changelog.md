...
- Gradle: builds that enable Gradle's configuration cache (`org.gradle.configuration-cache=true`, which projects generated by `gradle init` turn on) no longer fail analysis with `Invocation of 'Task.project' by task ':jsonDeps' at execution time is unsupported with the configuration cache`; the init script now resolves dependencies once the task graph is ready instead of inside the task action. ([#1790](https://github.com/fossas/fossa-cli/pull/1790))
