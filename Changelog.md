...
- Npm: `package-lock.json` files without a top-level `dependencies` key (generated by npm for projects with no dependencies) no longer fail analysis with `Error in $: key "dependencies" not found`.
