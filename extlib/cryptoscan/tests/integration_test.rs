...
fn has_algorithm(findings: &[Value], name: &str) -> bool {
    findings
        .iter()
        .any(|f| f["algorithm"]["name"].as_str().unwrap_or("") == name)
}
...
fn python_detects_aes_gcm_api_call() {
    let findings = run_scan("python-web-app", "python");
    assert!(
        has_algorithm_with_method(&findings, "AES-GCM", "api-call"),
...
fn python_detects_chacha20() {
    let findings = run_scan("python-web-app", "python");
    assert!(
        has_algorithm_with_status(&findings, "ChaCha20-Poly1305", "not-approved"),
...
fn python_detects_ecdsa_p256() {
    let findings = run_scan("python-web-app", "python");
    assert!(
        has_algorithm(&findings, "ECDSA-P256"),
...
fn python_detects_bcrypt() {
    let findings = run_scan("python-web-app", "python");
    assert!(
        has_algorithm(&findings, "bcrypt"),
        "should detect bcrypt import"
    );
}
...
fn python_detects_dependency_manifests() {
    let findings = run_scan("python-web-app", "python");
    let dep_findings: Vec<&Value> = findings
        .iter()
        .filter(|f| f["detection_method"].as_str() == Some("dependency-manifest"))
        .collect();
    assert!(
        dep_findings.len() >= 3,
        "should detect at least 3 dependency entries"
    );
}
...
fn python_total_finding_count() {
    ...
    assert!(
        findings.len() >= 20,
        "Python fixture should produce at least 20 findings, got {}",
        findings.len()
    );
}
...
fn java_detects_aes_gcm() {
    ...
    assert!(
        has_algorithm_with_status(&findings, "AES-256-GCM", "approved"),
...
fn java_detects_aes_ecb_deprecated() {
    let findings = run_scan("java-microservice", "java");
    assert!(
        has_algorithm_with_status(&findings, "AES-ECB", "deprecated"),
...
fn java_detects_3des_deprecated() {
    let findings = run_scan("java-microservice", "java");
    assert!(
        has_algorithm_with_status(&findings, "3DES", "deprecated"),
...
fn java_detects_bouncycastle_dependency() {
    let findings = run_scan("java-microservice", "java");
    assert!(
        has_algorithm_with_method(&findings, "BouncyCastle", "dependency-manifest"),
...
fn java_total_finding_count() {
    ...
    assert!(
        findings.len() >= 15,
        "Java fixture should produce at least 15 findings, got {}",
        findings.len()
    );
}
...
fn go_detects_ecdsa_curves() {
    ...
    assert!(
        has_algorithm(&findings, "ECDSA-P256"),
        "should detect P-256 curve"
    );
    assert!(
        has_algorithm(&findings, "ECDSA-P384"),
        "should detect P-384 curve"
    );
}
...
fn go_total_finding_count() {
    ...
    assert!(
        findings.len() >= 30,
        "Go fixture should produce at least 30 findings, got {}",
        findings.len()
    );
}
...
fn node_detects_package_json_deps() {
    let findings = run_scan("node-auth-service", "node");
    assert!(has_algorithm_with_method(
        &findings,
        "bcrypt",
        "dependency-manifest"
    ));
...
fn node_total_finding_count() {
    ...
    assert!(
        findings.len() >= 15,
        "Node fixture should produce at least 15 findings, got {}",
        findings.len()
    );
}
...
fn rust_detects_cargo_deps() {
    ...
    assert!(
        dep_findings.len() >= 5,
        "should detect at least 5 Cargo.toml deps, got {}",
        dep_findings.len()
    );
}
...
fn rust_detects_ring_dependency() {
    let findings = run_scan("rust-crypto-tool", "rust");
    assert!(
        has_algorithm_with_method(&findings, "ring", "dependency-manifest"),
...
fn rust_total_finding_count() {
    ...
    assert!(
        findings.len() >= 15,
        "Rust fixture should produce at least 15 findings, got {}",
        findings.len()
    );
}
...
fn cyclonedx_contains_crypto_components() {
    ...
    let crypto_components: Vec<&Value> = components
        .iter()
        .filter(|c| c["type"] == "cryptographic-asset")
        .collect();
    assert!(
        !crypto_components.is_empty(),
        "should contain cryptographic-asset components"
    );

    // Verify crypto properties structure
    let first_crypto = crypto_components[0];
    assert!(first_crypto["cryptoProperties"]["assetType"]
        .as_str()
        .is_some());
    assert!(
        first_crypto["cryptoProperties"]["algorithmProperties"]["primitive"]
            .as_str()
            .is_some()
    );
    assert!(
        first_crypto["cryptoProperties"]["algorithmProperties"]["algorithmFamily"]
            .as_str()
            .is_some()
    );
}
...
fn cyclonedx_contains_fossa_properties() {
    ...
    let crypto_component = components
        .iter()
    ...
    assert!(
        prop_names.contains(&"fossa:fips-status"),
        "should include fossa:fips-status"
    );
    assert!(
        prop_names.contains(&"fossa:detected-in"),
        "should include fossa:detected-in"
    );
    assert!(
        prop_names.contains(&"fossa:detection-method"),
        "should include fossa:detection-method"
    );
    assert!(
        prop_names.contains(&"fossa:ecosystem"),
        "should include fossa:ecosystem"
    );
}
...
fn cyclonedx_contains_dependencies() {
    ...
    assert!(
        deps.is_some(),
        "CycloneDX output should contain dependencies section"
    );
}
...
fn auto_ecosystem_detection() {
    ...
    assert!(
        !findings.is_empty(),
        "auto-detection should find the Python ecosystem"
    );
}
...
fn finding_includes_file_location() {
    ...
    assert!(
        api_finding["file_path"].as_str().is_some(),
        "should have file_path"
    );
    assert!(
        api_finding["line_number"].as_u64().unwrap() > 0,
        "should have positive line_number"
    );
    assert!(
        api_finding["matched_text"].as_str().is_some(),
        "should have matched_text"
    );
}
...
fn finding_includes_algorithm_details() {
    ...
    assert!(
        algo["classical_security_level"].is_null(),
        "AES-GCM without key size should not guess security level"
    );
