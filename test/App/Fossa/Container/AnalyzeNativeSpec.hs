...
analyzeSpec = describe "analyze" $ do
  ...
  it' "should not analyze application dependencies when only-system-dependencies are requested" $ do
    containerScan <- analyzeFromDockerArchive (UseGitBackedCargoLocators False) True (toFlag' False) mempty (toFlag' False) imageArchive
    buildImportsOf containerScan `shouldNotContain'` [numpy, scipy, black]

  it' "should analyze application dependencies" $ do
    containerScan <- analyzeFromDockerArchive (UseGitBackedCargoLocators False) False (toFlag' False) mempty (toFlag' False) imageArchive
  ...
  it' "should apply tool exclusion filter" $ do
    containerScan <- analyzeFromDockerArchive (UseGitBackedCargoLocators False) False (toFlag' False) (excludeTool SetuptoolsProjectType) (toFlag' False) imageArchive
    buildImportsOf containerScan `shouldNotContain'` [numpy, scipy, black]

  it' "should apply project exclusion filter" $ do
    let appAPath = $(mkRelDir "app/services/b/")
    containerScan <- analyzeFromDockerArchive (UseGitBackedCargoLocators False) False (toFlag' False) (excludeProject SetuptoolsProjectType appAPath) (toFlag' False) imageArchive
  ...
  it' "should analyze all targets, if default filter is disabled" $ do
    containerScan <- analyzeFromDockerArchive (UseGitBackedCargoLocators False) False (toFlag' False) mempty (toFlag' True) imageArchive
  ...
  it' "should apply path exclusion filter" $ do
    ...
    containerScan <- analyzeFromDockerArchive (UseGitBackedCargoLocators False) False (toFlag' False) (excludePath appAPath) (toFlag' False) imageArchive
  ...
  it' "should get os-info from any layer" $ do
    containerScan <- analyzeFromDockerArchive (UseGitBackedCargoLocators False) False (toFlag' False) mempty (toFlag' False) osInfoArchive
...
jarsInContainerSpec = describe "Jars in Containers" $ do
  ...
  it' "Reads and merges the layers correctly" $ do
    ContainerScan{imageData = ContainerScanImage{imageLayers}} <- analyzeFromDockerArchive (UseGitBackedCargoLocators False) False (toFlag' False) mempty (toFlag' False) imageArchivePath
