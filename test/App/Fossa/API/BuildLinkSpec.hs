  ...
  describe "BuildLink" $ do
    describe "SAML URL builder" $ do
      it' "should render simple locators" $ do
        let locator = Locator "fetcher123" "project123" $ Just "revision123"
            org = Just $ Organization (OrgId 1) True False True CLILicenseScan True True True False False False False [] False
      ...
      it' "should render git@ locators" $ do
        let locator = Locator "fetcher@123/abc" "git@github.com/user/repo" $ Just "revision@123/abc"
            org = Just $ Organization (OrgId 103) True False True CLILicenseScan True True True False False False False [] False
      ...
      it' "should render full url correctly" $ do
        let locator = Locator "a" "b" $ Just "c"
            org = Just $ Organization (OrgId 33) True False True CLILicenseScan True True True False False False False [] False
    ...
    describe "Fossa URL Builder" $
      it' "should render from API info" $ do
        ...
        GetOrganization `returnsOnce` Organization (OrgId 1) True False True CLILicenseScan True True True False False False False [] False
        let locator = Locator "fetcher123" "project123" $ Just "revision123"
            revision = ProjectRevision "" "not this revision" $ Just "master123"
