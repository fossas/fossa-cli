...
# Test raw project analysis:
echo "Testing bifrost-framework"
cd $HOME/bifrost-framework
cat <<EOF > .fossa.yml
...
cli:
  ...
  fetcher: git
  project: git@github.com:bifrost/bifrost-framework.git
analyze:
  modules:
  - name: bifrost
    target: .
